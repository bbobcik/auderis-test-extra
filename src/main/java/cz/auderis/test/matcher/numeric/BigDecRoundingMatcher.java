...
public class BigDecRoundingMatcher extends TypeSafeMatcher<BigDecimal> {
    ...
    protected boolean matchesSafely(BigDecimal num) {
        final BigDecimal roundedNum = num.setScale(roundingScale, roundingMode);
        return roundedValueMatcher.matches(roundedNum);
    }

    @Override
    public void describeTo(Description description) {
        description.appendText("decimal number that is ");
        roundedValueMatcher.describeTo(description);
        description.appendText(" when its scale is " + roundingScale);
        if (RoundingMode.UNNECESSARY != roundingMode) {
            description.appendText(" with rounding mode " + roundingMode);
        }
    }
    ...
    protected void describeMismatchSafely(BigDecimal num, Description out) {
        final BigDecimal roundedNum = num.setScale(roundingScale, roundingMode);
        roundedValueMatcher.describeMismatch(roundedNum, out);
    }
